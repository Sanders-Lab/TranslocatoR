...
Simply run `devtools::install_github(repo = "https://github.com/Sanders-Lab/TranslocatoR", ref = "master")`, load the TranslocatoR library with `library(TranslocatoR)` and you're ready to go. Have fun!
...
Clone the whole repository into your desired local directory by navigating to it, then running `git clone --recurse-submodules https://github.com/Sanders-Lab/TranslocatoR.git`. Inside Terminal or RStudio, navigate to the directory that now contains the cloned repo and set the `TranslocatoR` directory as your working directory. You can now run `devtools::install()` and TranslocatoR will be installed as normal. Don't forget to run `library(TranslocatoR)` and then you can then follow the usage example (keeping your working directory set to the cloned TranslocatoR repository folder).
